...
def evaluate_agent(agent: DDPGAgent, env: PortfolioEnv, dates: List[Any], render: bool = False) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        dates: Lista de fechas asociadas a los datos
...
def plot_portfolio_value(
    ...
    plt.plot(portfolio_df.index, portfolio_df['Value'], linewidth=2, color='#2ca02c')  # Verde para DDPG
...
def plot_weights_history(
    weights_df: pd.DataFrame,
    save_path: Optional[str] = None,
    title: str = 'Evolución de los Pesos de la Cartera - DDPG'
) -> None:
    """
    Visualiza la evolución de los pesos de la cartera a lo largo del tiempo.
    
    Args:
        weights_df: DataFrame con los pesos en cada paso
        save_path: Ruta para guardar el gráfico (opcional)
        title: Título del gráfico
    """
    if weights_df.empty:
...
def plot_crisis_analysis(crisis_analysis: Dict[str, Dict[str, Any]], results_dir: str) -> None:
    ...
    for i, (crisis_name, analysis) in enumerate(crisis_analysis.items()):
        if not analysis['drawdowns'].empty:
            plt.subplot(len(crisis_analysis), 1, i+1)
            analysis['drawdowns'].plot(color=colors[i % len(colors)])
    ...
    for i, (crisis_name, analysis) in enumerate(crisis_analysis.items()):
        if len(analysis['data']) > 1:  # Asegurar suficientes datos
            ...
            sns.histplot(returns, kde=True, color=colors[i % len(colors)])
    ...
    for i, (crisis_name, analysis) in enumerate(crisis_analysis.items()):
        if len(analysis['data']) > 1:
            ...
            normalized_returns.plot(color=colors[i % len(colors)], linewidth=2)
    ...
    for i, (crisis_name, analysis) in enumerate(crisis_analysis.items()):
        if len(analysis['data']) > 20:  # Necesitamos suficientes datos para la ventana
            plt.subplot(len(crisis_analysis), 1, i+1)
            returns = analysis['data']['Return'].iloc[1:]  # Eliminar el primer valor (cero)
            rolling_vol = returns.rolling(window=min(20, len(returns))).std() * np.sqrt(252) * 100  # Anualizada y en porcentaje
            rolling_vol.plot(color=colors[i % len(colors)], linewidth=2)
    ...
    # 5. Visualización de pesos durante crisis
    for crisis_name, analysis in crisis_analysis.items():
        if not analysis['weights'].empty and len(analysis['weights'].columns) > 0:
            plt.figure(figsize=(12, 6))
            plt.stackplot(
                analysis['weights'].index,
                [analysis['weights'][col] for col in analysis['weights'].columns],
                labels=analysis['weights'].columns,
                alpha=0.8
            )
            plt.title(f'Evolución de los Pesos durante {crisis_name} - DDPG')
            plt.xlabel('Fecha')
            ...
            crisis_name_safe = crisis_name.replace(" ", "_").lower()
            plt.savefig(os.path.join(crisis_dir, f'weights_during_{crisis_name_safe}.png'))
            plt.close()
...
def save_crisis_metrics(crisis_analysis: Dict[str, Dict[str, Any]], full_period_metrics: Dict[str, float], 
    ...
    with open(os.path.join(crisis_dir, 'crisis_analysis_summary.txt'), 'w') as f:
        f.write("ANÁLISIS DE CARTERA DDPG DURANTE PERIODOS DE CRISIS\n")
        f.write("================================================\n\n")
        ...
        if len(crisis_analysis) > 0:
            ...
            # Evalúa si el agente DDPG se desempeñó mejor que el periodo completo en alguna crisis
            better_than_full = []
            ...
            if better_than_full:
                f.write(f"3. El agente DDPG se desempeñó mejor que el periodo completo durante: {', '.join(better_than_full)}.\n")
            else:
                f.write("3. El agente DDPG no superó el rendimiento del periodo completo durante ninguna crisis.\n")
            ...
            if 'volatility' in metrics_comparison.columns and len(crisis_analysis) > 0:
                ...
                if crisis_vol:
                    ...
                    f.write(f"El agente DDPG experimentó mayor volatilidad durante {max_vol_crisis[0]} ({max_vol_crisis[1]:.2f}%).\n")
                else:
...
def create_risk_return_plot(metrics_comparison: pd.DataFrame, results_dir: str) -> None:
   """
   Crea un gráfico de rendimiento vs riesgo con los datos disponibles.
   
   Args:
       metrics_comparison: DataFrame con las métricas por periodo
       results_dir: Directorio donde guardar el gráfico
   """
   if 'volatility' not in metrics_comparison.columns or 'annual_return' not in metrics_comparison.columns:
       print("No hay suficientes datos para crear el gráfico de rendimiento vs riesgo")
       return
   ...
   # Graficar puntos para cada periodo
   for i, (period, row) in enumerate(metrics_comparison.iterrows()):
       volatility = row['volatility']
       return_val = row['annual_return']
       
       marker_size = 150 if period == 'Periodo Completo' else 100
       color_idx = 0 if period == 'Periodo Completo' else (i % (len(colors)-1)) + 1
       
       plt.scatter(volatility, return_val, s=marker_size, c=colors[color_idx], alpha=0.7, label=period)
       plt.annotate(period, (volatility, return_val), xytext=(10, 0), textcoords='offset points')
   
   # Línea de referencia Sharpe = 1
   x_range = np.linspace(0, metrics_comparison['volatility'].max() * 1.1, 100)
   ...
   plt.savefig(os.path.join(crisis_dir, 'risk_return_analysis.png'))
   plt.close()


def create_normalized_comparison_plot(crisis_analysis: Dict[str, Dict[str, Any]], 
                                      portfolio_df: pd.DataFrame, 
                                      results_dir: str) -> None:
   """
   Crea un gráfico comparativo de rendimientos normalizados entre periodos.
   
   Args:
       crisis_analysis: Resultados del análisis de crisis
       portfolio_df: DataFrame con los valores del portafolio completo
       results_dir: Directorio donde guardar el gráfico
   """
   ...
   plt.savefig(os.path.join(crisis_dir, 'normalized_comparison.png'))
   plt.close()

def main() -> int:
   """Función principal del evaluador de DDPG con análisis de crisis"""
   ...
   try:
       ...
       results_dir = os.path.join("results", f"ddpg_crisis_analysis_{timestamp}")
       ...
       # Evaluar agente en el periodo completo
       print("\nEvaluando agente DDPG en periodo completo...")
       ...
       # Mostrar resumen de resultados del periodo completo
       ...
       # Generar gráfico del periodo completo con periodos de crisis destacados
       print("\nGenerando gráficos de análisis...")
       plot_portfolio_value(
           portfolio_df,
           title='Evolución del Valor de la Cartera DDPG con Periodos de Crisis',
           highlight_crisis=True,
           save_path=os.path.join(results_dir, 'portfolio_value_with_crisis.png')
       )
       
       # Visualizar pesos de la cartera del periodo completo
       if not weights_df.empty:
           plot_weights_history(
               weights_df,
               save_path=os.path.join(results_dir, 'weights_evolution.png'),
               title='Evolución de los Pesos de la Cartera DDPG - Periodo Completo'
           )
       ...
       # Crear gráfico de rendimiento vs riesgo
       all_metrics = {'Periodo Completo': full_period_metrics}
       for crisis_name, analysis in crisis_analysis.items():
           all_metrics[crisis_name] = analysis['metrics']
       metrics_comparison = pd.DataFrame(all_metrics).T
       
       if not metrics_comparison.empty and 'volatility' in metrics_comparison.columns:
           create_risk_return_plot(metrics_comparison, results_dir)
       
       # Crear gráfico de rendimientos normalizados
       create_normalized_comparison_plot(crisis_analysis, portfolio_df, results_dir)
       ...
       for crisis_name, analysis in crisis_analysis.items():
           ...
           if not crisis_data.empty:
               ...
               plt.plot(crisis_data.index, crisis_data['Value'], linewidth=2, color='#2ca02c')
               plt.title(f'Valor de la Cartera DDPG durante {crisis_name}')
               plt.xlabel('Fecha')
               plt.ylabel('Valor ($)')
               plt.grid(True, alpha=0.3)
               plt.tight_layout()
