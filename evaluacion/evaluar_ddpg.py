...
def find_latest_model(model_type='ddpg'):
    """
    Encuentra el modelo DDPG más reciente en la carpeta 'modelos'.
    """
    model_dirs = [d for d in os.listdir('modelos') if d.startswith(f'{model_type}_')]
    
    if not model_dirs:
        print(f"No se encontraron modelos {model_type.upper()}. Por favor, especifica la ruta manualmente.")
        return None
...
def plot_portfolio_value(portfolio_values, dates, save_path=None):
    ...
    plt.title('Evolución del Valor de la Cartera - DDPG')
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    model_path = find_latest_model(model_type='ddpg')
    if not model_path:
    ...
    agent = DDPGAgent(
        ...
        hidden_dim=64,  # Usar el mismo número que en el entrenamiento
